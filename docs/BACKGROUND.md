...
保存诊断开关、Key 或渠道后，言外会向运行中的微信发送完整设置。广播限定微信包名，只接受拥有言外签名权限的发送者；Provider 原有 UID 白名单保留。设置带单调递增版本，延迟到达的旧广播不能覆盖较新的配置或恢复已清空的 Key。模型配置只缓存于内存，不另写入微信文件或备份。当前会话分析开关另外保存在微信本地私有设置中，不经过 Provider，默认关闭。未新增前台服务、通知、开机广播或后台轮询项目进程。
...
2. 关闭当前聊天右上角“分析”，划掉言外，再看新文字，确认不再发起分析。清空 Key 后也应停止。
