...
- 引用原文以独立 `quoted_message` 提供，保留显示名、内容类型和服务端引用标识；不把引用原作者当成当前发送者，不伪造时间或插入相邻时间序列。只读取 type=1 的引用文字；其他媒体、过长或缺失的引用仅标记不可用，不上传附件 XML，不读取附件。群成员前缀与 XML 中的冒号换行分别处理。
- 解析限制 64 KiB 输入、24 层及 512 个 XML 元素，拒绝 DTD、实体声明、重复关键字段和不支持的结构；短回复不因引用文字超过 1000 字符而被整体丢弃。引用文字计入既有上下文总预算，引用身份与内容进入缓存键及手动选择身份，防止改引用后复用旧结果。未扩展「帮我回」历史读取。
- 格式核对参考：[开源接入的引用字段处理](https://github.com/latorc/Wechat-AI-Assistant/blob/main/wcf_wrapper.py)、[消息类型定义](https://pkg.go.dev/github.com/Clov614/wcf-rpc-sdk)。仅用于核对 XML 和类型，生产解析独立实现；没有读取手机聊天数据或调用真实模型。
- 先运行新回归复现 4 个失败，再修复。最终全量离线测试 270 项通过，0 failures/errors/skipped；Debug 构建成功，使用 `--offline --no-daemon -Pkotlin.compiler.execution.strategy=in-process`。覆盖两种宿主类型、群聊归属、前文引用、非文字引用、实体/畸形 XML、字符转义、过长引用、缓存和上下文预算；独立静态审查未发现明确重要问题。
- ADB 覆盖安装 Success，包信息为 2.1.6 / 39。未重启或操作微信；用户彻底重启后测试私聊/群聊的引用文字回复、自动开关与长按分析，实际宿主 XML 和卡片布局仍待实机验收。结束时未发现项目服务或 Gradle/Kotlin 编译进程残留，仅本地提交。

---

# 2.1.5 隐藏零概率情绪（2026-09-27）

- 概率为 0 的情绪不再生成圆环、标签或占位，剩余项自然紧凑排列；非零小概率继续显示 `<1%`。全部为零时不残留概率文本行。固定配色、顺序和两条线路共用展示逻辑保持一致。
- 全量离线测试 262 项通过，0 failures/errors/skipped，Debug 构建成功；使用 `--offline --no-daemon -Pkotlin.compiler.execution.strategy=in-process`。既有测试同步验证仅非零项保留及全零为空。
- ADB 覆盖安装 Success，包信息确认 2.1.5 / 38。未操作微信，重启后的布局由用户验收；未发现项目服务或 Gradle/Kotlin 编译进程残留，仅本地提交。

---

# 2.1.4 彩色情绪小圆环（2026-09-27）

- 两条线路的分析卡均以约 24dp 静态圆环展示情绪概率，环内显示百分比，旁边保留名称。10 种情绪固定配色和顺序，包含 0% 空环；非零极小概率与接近 100% 的概率使用 `<1%` / `>99%`，避免四舍五入冒充绝对判断。弧长使用实际概率。
- 各项独立 inline span，按可用宽度换行，保留原始文字供辅助阅读；大字模式随字号增大，单项宽于卡片时退回文字。首行 yanwai、短句解读、仅供参考、透明背景和紧凑间距继续沿用，不增加动画或横向长条。
- 智能分析等待解读时保留结构化 JEV 情绪，提前显示相同圆环；完成后复用同一排情绪。失败、取消及配置变化沿用原有临时状态清理。独立静态复核未发现明确正确性问题。
- 离线 `:app:testDebugUnitTest :app:assembleDebug --offline --no-daemon -Pkotlin.compiler.execution.strategy=in-process` 成功；262 tests，0 failures/errors/skipped。新增覆盖情绪与颜色完整性、顺序稳定、零概率、临界舍入及非法数值保护。
- ADB 覆盖安装 Success，系统包信息为 2.1.4 / 37。未操作微信、未运行设备 UI 自动化，实际圆环密度、换行、系统大字与读屏由用户验收。结束时未发现项目服务或 Gradle/Kotlin 编译进程残留，仅本地提交。

---

# 2.1.3 分析卡紧凑排版（2026-09-27）

- 移除所有分析卡情绪概率条及其绘制代码，两条线路统一保留 JEV 原有文字概率；取消段落空行，上下内边距从 10dp 调整为 7dp，附加行距从 3dp 调整为 1dp，正文仍为 13sp 并支持系统字体缩放。
- 卡片、加载/失败提示及设置选项统一称为「智能分析」；结尾仅保留「仅供参考」。生成提示要求每项一句、优先 20–40 字，保留推测与证据不足措辞；展示合并模型多余换行，不硬截断内容。未实测模型是否始终遵循长度要求。
- 成功、加载及失败分析卡首行统一为 `yanwai {BuildConfig.VERSION_NAME}`，继续使用生成版本。产品、界面规范和 README 同步更新。
- 离线 `:app:testDebugUnitTest :app:assembleDebug --offline --no-daemon -Pkotlin.compiler.execution.strategy=in-process` 成功；259 tests，0 failures/errors/skipped。现有协议测试核对紧凑文案与动态版本标题；未新增设备 UI 自动化。
- ADB 覆盖安装 Success，系统包信息确认 2.1.3 / 36。没有重启或操作微信；用户彻底重启微信后验收卡片密度、概率文字和两种线路。结束时未发现项目服务或 Gradle/Kotlin 编译进程残留，仅本地提交。
