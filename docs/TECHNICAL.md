...
- 分析目标接受微信消息对象 `field_type == 1` 的非空文字及 `field_type == 34` 的语音，方向均须 `field_isSend == 0`。语音先经微信原生转写，不能把原始 content 媒体载荷当正文。无法确认类型或会话的行跳过。
- 每条正文超过 **1000 字符**直接跳过，不截断分析，也不作为上下文；按 Unicode 码点计数（普通 emoji 不按两个字符算），群聊发送者前缀不计入正文。
- 文件名、图片、视频、表情包、链接卡片、引用卡片和系统消息不作为正文；语音仅纳入转写文字，不分析音调或声学情绪。
- 仅为屏幕可见消息发起分析；从目标之前向前最多扫描 **80 个已加载位置**，收集最近 **24 条文字或语音、转写后 12000 字符**。保留整条文字和发送者，按旧到新排列；其他媒体不占名额，但记录未读取内容、异常时间、失败与截断。已知时间晚于目标或逆序的前文不进入分析。语音仅按已选定消息身份查询原生转写，不扩展分析历史窗口，不读取目标之后的消息。详见[语音支持](VOICE_SUPPORT.md)。
- `AnalysisState` 在代码中计算时间差和本地跨日标记，缺失时间显式为 null。两小时间隔分时间块、同发送者两分钟内文字分组，都仅提供结构，不判定换话题或消气。两轮使用同一份输入快照；时间、前文条数、范围和缺口只用于分析，不显示在卡片中。
...
- 保留微信原消息行及 ViewHolder，在已识别的文字或语音气泡下追加独立分析卡，列表回收前移除；未知布局跳过并报告。语音卡显示准备进度、失败重试或分析结果及转写正文。
