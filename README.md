...
个人使用的 Android LSPosed / Xposed 模块，当前版本 **0.4.1**。
...
- 每条目标仍只发 **一次 Jev 请求**，同时判断情绪、意图、风险和建议；前文帮助理解当前消息，不作为独立分析目标。建议仍从固定 5 个选项中选择。
...
如果没有「已加载」提示，也没有设置入口，先查看 LSPosed 是否出现 `WeChatMood 0.4.1: entered WeChat main process`。没有这条日志说明尚无进入模块入口的证据，不能靠检测模型连接判断注入是否成功。

上次实机微信版本为 8.0.71。0.4.0 已实机确认标题栏开关、气泡下方分析卡、关闭绘制恢复布局；0.4.1 的上下文效果由用户覆盖安装并重启微信后验收。验证记录见 [docs/VERIFY.md](docs/VERIFY.md)。
