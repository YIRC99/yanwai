...
![Version](https://img.shields.io/badge/version-2.0.1-2C7A73)
...
<table>
  ...
  <tr>
    ...
    <td>从真实前文延伸话题，查看理由，选中一句复制或填入。</td>
  </tr>
</table>
...
- **两轮复核**：第二轮复核情绪、意图和建议前提。识别到暂停或拒绝时，不继续给出劝说建议；依据不足时可以只展示情绪。
...
打开微信聊天「＋」面板 → **帮我回** → 选择身份与参考条数 → 补充想法 → **生成回复** → 选中一句复制或填入 → 自己发送。
...
| 分条回复 | 按语意生成 1–6 条短消息；逐条选择、复制或填入，适合微信里自然的停顿和接话 |
| 查看依据 | 查看建议实际参考的消息、说话人、时间和来源；历史不足时显示实际读取数量 |
| 重开继续 | 保留当前微信进程内上次成功建议，填入后重开可继续下一条；不会覆盖尚未发送或清空的输入框内容 |
...
- 话题里的短句同样可逐条复制、填入；五个话题是备选方向，不是让你一次全发。
- 身份、上下文或补充想法变化后，需要重新生成，不把旧话题当成新背景下的建议。

### 情绪判断与回复模型，分别配置

设置页分为 **情绪判断 / 回复建议 / 关于言外**，两套模型配置独立，**只想使用「帮我回」也可以只配回复模型**。
...
2. 按需要配置功能：在「情绪判断」选择渠道、填写 Key 并保存检测；在「回复建议」选择供应商、填写 Key 和模型，开启「允许手动生成回复建议」后保存检测。两项不必同时开启。
...
- 只为**当前屏幕可见的对方文字或语音**发起情绪分析；单条正文或语音转写超过 **1000 字符**时跳过，不截断后给出结论。
...
不会。填入只放到微信输入框，你可以修改后再发送。已有未发送内容时，不会用下一条建议直接覆盖。
