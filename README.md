...
![Version](https://img.shields.io/badge/version-1.0.0-2C7A73)
...
- **自己配置模型接口**：填写 API 地址和 Key，默认使用 Jev 官方接口，APK 不携带个人密钥。
...
以上为作者提供的实机使用截图，拍摄于 **0.5.2**，点击可查看原图。当前版本为 **1.0.0**，新增用户 API 配置并采用「言外」名称；截图中的旧版本文字保留原样。
...
- 自己的 **Jev API Key**，以及可访问模型接口的网络。
- 项目已有实机记录的微信版本为 **8.0.71**，其他版本的界面适配需实际验证。

### 安装与配置

1. 安装 APK，先从桌面打开一次 **「言外」**。
2. 填写 **API 地址**和 **API Key**，点击 **「保存并检测连接」**。地址已默认填好，只使用官方服务时填写 Key 即可。
...
| 配置项 | 说明 |
| --- | --- |
| API 地址 | 默认 `https://api.typesafe.ai/v1/systemone` |
| API Key | 填写从 Jev 官网获取的 Key；自定义服务使用该服务对应的 Key |
| 模型 | 固定为 `jev-1.13.0`，无需额外填写 |

支持保存其他完整 HTTP(S) 地址，程序不会自动补接口路径。建议使用 HTTPS；HTTP 在微信进程中还会受到宿主网络策略限制。

**自定义服务需要兼容 Jev 专用请求与概率响应协议。** 1.0.0 尚未实现 OpenRouter 或通用 OpenAI 接口适配，单纯替换地址不能保证使用。连接检测仅发送内置示例，不读取你的聊天；检测成功也不代表微信模块已经加载。
...
1.0.0 已完成 64 项单元测试、Debug / Release 构建和 lint 检查。完整构建遇到旧版本缓存时，参考[验证记录](docs/VERIFY.md)中的无缓存构建命令。自动化检查不代表所有微信版本均兼容。
