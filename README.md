...
设置页顶部切换「情绪判断 / 回复建议」。情绪判断继续使用 Jev；回复建议选择 **豆包、小米 MiMo、DeepSeek、OpenAI GPT、GLM、Kimi 或自定义**，填 Key 后获取并选择模型，再检测回复。每家配置独立保存；支持手动填模型和自定义 OpenAI 兼容地址。开启手动生成许可并保存后可在微信使用。

微信聊天「＋」面板点「帮我回」，结合最近最多 100 条已加载聊天生成一句回复。抽屉默认展示理由，支持查看上下文、换一句、更简短、补充想法、复制及填入输入框。关闭重开保留上次成功建议，不会自动重新生成；微信进程结束后清空。填入后自己修改并发送，重新打开可撤销。新消息仅提示更新，切换聊天或关闭面板取消未完成请求。加号布局待手机验收，长按「分析」→「帮我回 / 上次建议」可作备用入口。

**回复逻辑来自 [狗头军师 goutoujunshi](https://github.com/shengjidaguai-china/goutoujunshi)**。完整原始知识随应用打包，保留 Copyright (c) 2026 powerycy 和 MIT 许可；由你配置的长上下文模型生成建议。聊天片段、时间、说话人和草稿仅在主动生成时送到所选服务商，不读取完整聊天数据库。详细说明和手机验收见 [回复建议](docs/REPLY_ASSISTANT.md)。
...
以上为作者提供的实机使用截图，拍摄于 **0.5.2**，点击可查看原图。当前版本为 **1.5.1**，顶部名称旁显示实际安装版本，并加入专属桌面图标。支持单条消息「翻译意图」、各聊天开关记忆、日志导出、多渠道接入和更新提醒，并新增独立 LLM 回复建议与供应商模型选择。截图中的旧版本文字保留原样；新增功能待手机验收。
