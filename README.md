...
微信聊天「＋」面板点「帮我回」，主动读取当前聊天最近最多 100 条本机文字记录生成一句回复，不依赖首次加载的十几条消息。历史不可用时明确标注仅页面消息；可查看实际条数、来源、时间及原文。抽屉默认展示理由，支持换一句、更简短、补充想法、复制及填入输入框。关闭重开保留上次成功建议，不会自动重新生成；微信进程结束后清空。填入后自己修改并发送，重新打开可撤销。新消息仅提示更新，切换聊天或关闭面板取消未完成请求。加号布局与历史读取需手机验收，长按「分析」→「帮我回 / 上次建议」可作备用入口。

**回复逻辑来自 [狗头军师 goutoujunshi](https://github.com/shengjidaguai-china/goutoujunshi)**。完整原始知识随应用打包，保留 Copyright (c) 2026 powerycy 和 MIT 许可；由你配置的长上下文模型生成建议。聊天片段、时间、说话人和草稿仅在主动生成时送到所选服务商；按当前聊天限量读取，不遍历联系人或导出数据库。详细说明和手机验收见 [回复建议](docs/REPLY_ASSISTANT.md)。
