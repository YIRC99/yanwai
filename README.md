...
![Version](https://img.shields.io/badge/version-1.4.0-2C7A73)
...
设置页顶部切换「情绪判断 / 回复建议」。情绪判断继续使用 Jev；回复建议独立填写 **OpenAI 兼容 Chat Completions 地址、API Key、模型名**，开启手动生成许可并保存。
...
以上为作者提供的实机使用截图，拍摄于 **0.5.2**，点击可查看原图。当前版本为 **1.4.0**，顶部名称旁显示实际安装版本，并加入专属桌面图标。支持单条消息「翻译意图」、各聊天开关记忆、日志导出、多渠道接入和更新提醒，并新增独立 LLM 回复建议。截图中的旧版本文字保留原样；新增功能待手机验收。
