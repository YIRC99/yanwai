...
![Version](https://img.shields.io/badge/version-2.1.0-2C7A73)
...
- **两条意图线路**：默认 JEV 决策模型，从内置选项快速判断；也可选通用大模型，结合前文解释意图、可能在意的点和情绪倾向。LLM 通常更慢，也可能误读，不保证更准确；这条线路不生成回应建议。两种线路的情绪概率始终由 JEV 提供。
- **深色透明卡片**：前三项情绪用概率条显示，其余概率单列合计；分段解读放在原消息下方。背景模糊不可用时退回半透明样式，文字保持清晰。新样式尚待实机验收，上方截图仍为 2.0.0。
