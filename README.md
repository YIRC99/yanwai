...
个人使用的 Android LSPosed / Xposed 模块，当前版本 **0.5.0**。

对微信当前屏幕中**对方的纯文本消息**调用 Jev，解读闲聊事件、情绪、好感线索和可能的潜台词，并给出下一步沟通建议。内置 8 类场景、32 套分析卡，卡片随解释、行动、接受和收尾变化。模型判断仅供参考，不发送消息、不自动回复。
...
- 正常每条目标发 **两次串行 Jev 请求**：第一层判断场景、情绪、互动意愿和对话进展；第二层针对适用模板判断潜台词、选择分析卡及原话参考，代码组合展示文案和对应动作。每个问题独立，不依赖同轮其他问题的答案。
- 第一层不确定、不属于闲聊或无适用模板时只调用一次，显示克制的提示；已有相同输入缓存时不再调用。第二层也保留普通解释、信息不足及不适用选项，不能为了趣味强行猜测。好感线索是可观察的交流意愿，不是恋爱概率；群聊显示「互动线索」。
...
如果没有「已加载」提示，也没有设置入口，先查看 LSPosed 是否出现 `WeChatMood 0.5.0: entered WeChat main process`。没有这条日志说明尚无进入模块入口的证据，不能靠检测模型连接判断注入是否成功。

上次实机微信版本为 8.0.71。0.4.0 已实机确认标题栏开关、气泡下方分析卡、关闭绘制恢复布局。0.5.0 的闲聊解读质量、概率表现和卡片长度由用户覆盖安装并重启微信后验收；本轮离线测试不代表真实模型或实机效果。验证记录见 [docs/VERIFY.md](docs/VERIFY.md)。
