...
| 操作 | 效果 |
| --- | --- |
| 开关聊天右上角「分析」 | 控制当前会话的自动分析并本地记住；关闭后隐藏自动分析卡、停止自动请求，手动选中的消息仍保留 |
...
- 手动选中的消息与结果只保留在当前微信进程内，切换聊天后返回可恢复，重启微信后清空。加载更多历史不会自动重做已选消息；没有可靠消息标识时不添加单条入口。
...
会停止当前会话的自动分析，隐藏自动分析结果，排队的自动任务和未发出的第二轮也会停止。通过「翻译意图」手动选中的消息仍可分析和显示，已经发出的请求可能继续返回。其他联系人和群聊不受影响。旧版两个全局开关不再生效，升级后所有未单独开启的会话默认关闭。
